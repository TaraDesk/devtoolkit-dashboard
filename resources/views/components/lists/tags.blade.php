<div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <div id="alphabet-dropdown" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-10 p-2">
                    <div class="overflow-y-auto max-h-60">
                        <div class="grid grid-cols-5 gap-1.5 text-sm font-medium text-gray-500">
                            @foreach ($alphabet as $letter => $groupedTags)
    ...
    <div class="overflow-hidden">
        @if (count($tags))
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 p-6 gap-6" id="list-container">
                @foreach ($tags as $tag)
                    <x-cards.tag :tag="$tag"/>
                @endforeach
            </div>
        @else
            <div class="px-6 py-12">
                <div class="flex flex-col items-center justify-center gap-3 text-gray-400">
                    ...
                    <p class="text-sm font-medium text-gray-500">No tags found in the system</p>
                    <p class="text-xs text-gray-400">Start by adding new tags</p>
                </div>
            </div>
        @endif
    </div>
